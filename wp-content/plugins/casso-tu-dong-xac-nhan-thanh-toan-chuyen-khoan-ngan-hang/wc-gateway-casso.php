	...
	*/
	...
	public function woocommerce_casso_woocommerce_blocks_support()
	{
		...
		if (class_exists('Automattic\WooCommerce\Blocks\Payments\Integrations\AbstractPaymentMethodType')) {
			foreach ($active_bank_accounts as $account) {
				if (strtolower($account['bank_name']) == 'acb') {
					require_once dirname(__FILE__) . '/inc/banks/class-casso-acb-blocks-support.php';
					add_action(
						'woocommerce_blocks_payment_method_type_registration',
						function (Automattic\WooCommerce\Blocks\Payments\PaymentMethodRegistry $payment_method_registry) {
							$payment_method_registry->register(new WC_CASSO_ACB_Blocks_Support);
						}
					);
				}
				if (strtolower($account['bank_name']) == 'mbbank') {
					require_once dirname(__FILE__) . '/inc/banks/class-casso-mbbank-blocks-support.php';
					add_action(
						'woocommerce_blocks_payment_method_type_registration',
						function (Automattic\WooCommerce\Blocks\Payments\PaymentMethodRegistry $payment_method_registry) {
							$payment_method_registry->register(new WC_CASSO_MBBank_Blocks_Support);
						}
					);
				}
				if (strtolower($account['bank_name']) == 'techcombank') {
					require_once dirname(__FILE__) . '/inc/banks/class-casso-techcombank-blocks-support.php';
					add_action(
						'woocommerce_blocks_payment_method_type_registration',
						function (Automattic\WooCommerce\Blocks\Payments\PaymentMethodRegistry $payment_method_registry) {
							$payment_method_registry->register(new WC_CASSO_Techcombank_Blocks_Support);
						}
					);
				}
				if (strtolower($account['bank_name']) == 'timoplus') {
					require_once dirname(__FILE__) . '/inc/banks/class-casso-timoplus-blocks-support.php';
					add_action(
						'woocommerce_blocks_payment_method_type_registration',
						function (Automattic\WooCommerce\Blocks\Payments\PaymentMethodRegistry $payment_method_registry) {
							$payment_method_registry->register(new WC_CASSO_Timoplus_Blocks_Support);
						}
					);
				}
				if (strtolower($account['bank_name']) == 'vpbank') {
					require_once dirname(__FILE__) . '/inc/banks/class-casso-vpbank-blocks-support.php';
					add_action(
						'woocommerce_blocks_payment_method_type_registration',
						function (Automattic\WooCommerce\Blocks\Payments\PaymentMethodRegistry $payment_method_registry) {
							$payment_method_registry->register(new WC_CASSO_VPBank_Blocks_Support);
						}
					);
				}
				if (strtolower($account['bank_name']) == 'vietinbank') {
					require_once dirname(__FILE__) . '/inc/banks/class-casso-vietinbank-blocks-support.php';
					add_action(
						'woocommerce_blocks_payment_method_type_registration',
						function (Automattic\WooCommerce\Blocks\Payments\PaymentMethodRegistry $payment_method_registry) {
							$payment_method_registry->register(new WC_CASSO_Vietinbank_Blocks_Support);
						}
					);
				}
				if (strtolower($account['bank_name']) == 'ocb') {
					require_once dirname(__FILE__) . '/inc/banks/class-casso-ocb-blocks-support.php';
					add_action(
						'woocommerce_blocks_payment_method_type_registration',
						function (Automattic\WooCommerce\Blocks\Payments\PaymentMethodRegistry $payment_method_registry) {
							$payment_method_registry->register(new WC_CASSO_OCB_Blocks_Support);
						}
					);
				}
				if (strtolower($account['bank_name']) == 'tpbank') {
					require_once dirname(__FILE__) . '/inc/banks/class-casso-tpbank-blocks-support.php';
					add_action(
						'woocommerce_blocks_payment_method_type_registration',
						function (Automattic\WooCommerce\Blocks\Payments\PaymentMethodRegistry $payment_method_registry) {
							$payment_method_registry->register(new WC_CASSO_TPBank_Blocks_Support);
						}
					);
				}
				if (strtolower($account['bank_name']) == 'vietcombank') {
					require_once dirname(__FILE__) . '/inc/banks/class-casso-vietcombank-blocks-support.php';
					add_action(
						'woocommerce_blocks_payment_method_type_registration',
						function (Automattic\WooCommerce\Blocks\Payments\PaymentMethodRegistry $payment_method_registry) {
							$payment_method_registry->register(new WC_CASSO_Vietcombank_Blocks_Support);
						}
					);
				}
				if (strtolower($account['bank_name']) == 'bidv') {
					require_once dirname(__FILE__) . '/inc/banks/class-casso-bidv-blocks-support.php';
					add_action(
						'woocommerce_blocks_payment_method_type_registration',
						function (Automattic\WooCommerce\Blocks\Payments\PaymentMethodRegistry $payment_method_registry) {
							$payment_method_registry->register(new WC_CASSO_BIDV_Blocks_Support);
						}
					);
				}
				if (strtolower($account['bank_name']) == 'agribank') {
					require_once dirname(__FILE__) . '/inc/banks/class-casso-agribank-blocks-support.php';
					add_action(
						'woocommerce_blocks_payment_method_type_registration',
						function (Automattic\WooCommerce\Blocks\Payments\PaymentMethodRegistry $payment_method_registry) {
							$payment_method_registry->register(new WC_CASSO_Agribank_Blocks_Support);
						}
					);
				}
			}
